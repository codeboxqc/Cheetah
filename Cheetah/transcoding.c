...
/////////////////////////////////////////
typedef enum {
    ENCODE_H265_ARCHIVE = 1,   // H.265 Archive 
    ENCODE_H264_HIGH = 2,       // H.264 High 
    ENCODE_H264_BALANCED = 3,   // H.264 Balanced 
    ENCODE_H264_SMALL = 4       // H.264 Small 
} EncodingOption;
...
static int open_input_file(const char* filename)
{
    ...
    for (i = 0; i < ifmt_ctx->nb_streams; i++) {
        ...
        if (!dec) {
            log_debug("Skipping stream: Decoder not found");
            continue;
        }
        ...
        if (codec_ctx->codec_type == AVMEDIA_TYPE_VIDEO ||
            ...
            if (ret < 0) {
                ...
                log_debug("Skipping stream: Could not open decoder");
                continue;
            }
        }
    ...
    AVStream* in_stream, const AVCodec** encoder_out)
{
    ...
    if (strcmp(g_output_format, "webm") == 0) {
        ...
        if (encoder) {
            ...
            // CRITICAL FALLBACK: If user is missing vpx, they cannot make WebM.
            log_debug("CRITICAL ERROR: libvpx (VP8/VP9) not found! Please install ffmpeg[vpx].");
    ...
    if (!encoder) {
        log_debug("ERROR - No suitable video encoder found!");
        return AVERROR_ENCODER_NOT_FOUND;
    }
    ...
    *encoder_out = encoder;
    ...
    // 3. PIXEL FORMAT & QUALITY
    if (strstr(encoder->name, "libvpx") || encoder->id == AV_CODEC_ID_VP9 || encoder->id == AV_CODEC_ID_VP8) {
...
// VP9 (WebM) quality configuration
static void configure_vp9_quality(AVCodecContext* enc_ctx, EncodingOption option)
{
    // Important for VP9 speed!
    av_opt_set(enc_ctx->priv_data, "deadline", "good", 0);
    av_opt_set_int(enc_ctx->priv_data, "cpu-used", 3, 0); // 0-5

    // VP9 uses "crf" similar to x264
    switch (option) {
    case ENCODE_H265_ARCHIVE:
        av_opt_set_int(enc_ctx->priv_data, "crf", 20, 0);
        av_opt_set(enc_ctx->priv_data, "b", "0", 0);
        break;
    case ENCODE_H264_HIGH:
        av_opt_set_int(enc_ctx->priv_data, "crf", 28, 0);
        av_opt_set(enc_ctx->priv_data, "b", "0", 0);
        break;
    case ENCODE_H264_BALANCED:
        av_opt_set_int(enc_ctx->priv_data, "crf", 34, 0);
        av_opt_set(enc_ctx->priv_data, "b", "0", 0);
        break;
    case ENCODE_H264_SMALL:
        av_opt_set_int(enc_ctx->priv_data, "crf", 45, 0);
        av_opt_set(enc_ctx->priv_data, "b", "0", 0);
        break;
    }
}
...
static int open_output_file(const char* filename)
{
    ...
    int ret;
    unsigned int i;
    int video_stream_found = 0; // Track so we only encode ONE video stream
    ...
    for (i = 0; i < ifmt_ctx->nb_streams; i++) {
        ...
        // PREVENT DUAL VIDEO STREAMS (e.g. Cover Art)
        if (dec_ctx->codec_type == AVMEDIA_TYPE_VIDEO) {
            if (video_stream_found) {
                log_debug("Skipping secondary video stream (Cover Art?)");
                stream_ctx[i].out_stream_index = -1;
                continue;
            }
        ...
        if (dec_ctx->codec_type == AVMEDIA_TYPE_VIDEO) {
            ...
            if (ret < 0) {
                ...
                char msg[512];
                snprintf(msg, sizeof(msg), "Video encoder open failed: %s", err_buf);
                log_debug(msg);
                return ret;
            }
            ...
            if (!encoder) {
                log_debug("Audio encoder not found, skipping audio stream");
                continue;
            }
            ...
            // CRITICAL FIX: Opus specific settings
            if (encoder->id == AV_CODEC_ID_OPUS) {
                enc_ctx->bit_rate = 128000;
                // Opus strictly requires 48kHz (or 24/16/12/8). It does NOT support 44.1kHz.
                // We MUST set this here, and the filter graph (aresample) will handle conversion.
            ...
            if (ret < 0) {
                ...
                av_strerror(ret, err_buf, sizeof(err_buf));
                char msg[512];
                snprintf(msg, sizeof(msg), "Audio encoder open failed: %s", err_buf);
                log_debug(msg);
                return ret;
            }
    ...
    AVCodecContext* enc_ctx, const char* filter_spec)
{
    ...
    if (dec_ctx->codec_type == AVMEDIA_TYPE_VIDEO) {
        ...
        if (dec_ctx->pkt_timebase.den == 0) dec_ctx->pkt_timebase = (AVRational){ 1, 25 };
        if (dec_ctx->sample_aspect_ratio.den == 0) dec_ctx->sample_aspect_ratio = (AVRational){ 1, 1 };
        ...
        if (!buffersink_ctx) { ret = AVERROR(ENOMEM); goto end; }
    ...
    if ((ret = avfilter_graph_parse_ptr(filter_graph, filter_spec,
        &inputs, &outputs, NULL)) < 0) goto end;
...
static int init_filters(void)
{
    ...
    for (i = 0; i < ifmt_ctx->nb_streams; i++) {
        ...
        // Skip streams that weren't mapped to output
        if (stream_ctx[i].out_stream_index < 0 || !stream_ctx[i].enc_ctx) {
            continue;
        }

        // CRITICAL FIX FOR WEBM/OPUS CRASH:
        // Use "scale" for video (allows resizing/pixel format conversion like NV12->YUV420P)
        // Use "aresample" for audio (allows 44.1k -> 48k conversion required by Opus)
        if (ifmt_ctx->streams[i]->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
            filter_spec = "scale";
        }
        else {
            filter_spec = "aresample";
        }
...
static int encode_write_frame(unsigned int stream_index, int flush)
{
    ...
    while (ret >= 0) {
        ...
        if (stream->out_stream_index < 0) {
            av_packet_unref(enc_pkt);
            continue;
        }

        enc_pkt->stream_index = stream->out_stream_index;

        av_packet_rescale_ts(enc_pkt,
            stream->enc_ctx->time_base,
            ofmt_ctx->streams[stream->out_stream_index]->time_base);
...
int transcode_main(int argc, char** argv)
{
    ...
    for (i = 0; i < ifmt_ctx->nb_streams; i++) {
        if (stream_ctx[i].out_stream_index < 0) continue;
        if (!filter_ctx[i].filter_graph) continue;

        StreamContext* stream = &stream_ctx[i];
        if (!stream->dec_ctx || !stream->dec_frame) continue;
    ...
    if (stream_ctx) {
        for (i = 0; i < ifmt_ctx->nb_streams; i++) {
            if (stream_ctx[i].dec_ctx) avcodec_free_context(&stream_ctx[i].dec_ctx);
            if (stream_ctx[i].enc_ctx) avcodec_free_context(&stream_ctx[i].enc_ctx);
            if (stream_ctx[i].dec_frame) av_frame_free(&stream_ctx[i].dec_frame);
        }
        av_free(stream_ctx);
    }
    ...
    if (filter_ctx) {
        for (i = 0; i < ifmt_ctx->nb_streams; i++) {
            if (filter_ctx[i].filter_graph) avfilter_graph_free(&filter_ctx[i].filter_graph);
            if (filter_ctx[i].enc_pkt) av_packet_free(&filter_ctx[i].enc_pkt);
            if (filter_ctx[i].filtered_frame) av_frame_free(&filter_ctx[i].filtered_frame);
        }
        av_free(filter_ctx);
    }
